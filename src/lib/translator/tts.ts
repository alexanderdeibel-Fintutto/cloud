...
function getVoiceConfig(speechCode: string, quality: VoiceQuality = 'neural2') {
  ...
  if (quality === 'chirp3hd') return getVoiceConfig(speechCode, 'neural2')
  return { config: { languageCode: speechCode, name: '' }, useBeta: false }
}

export async function speakWithCloudTTS(text: string, speechCode: string, quality: VoiceQuality = 'neural2'): Promise<HTMLAudioElement> {
  if (!API_KEY) throw new Error('Google Cloud TTS API key not configured')
  ...
  const body: Record<string, unknown> = {
    input: { text },
    voice: { languageCode: voiceConfig.languageCode, ...(voiceConfig.name ? { name: voiceConfig.name } : {}) },
  ...
  if (!response.ok) {
    if (quality === 'chirp3hd') return speakWithCloudTTS(text, speechCode, 'neural2')
