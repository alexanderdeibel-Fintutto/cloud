...
import { ArrowLeft, ScrollText, Plus, Trash2, Save, FileDown } from 'lucide-react'
...
export default function HausordnungPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'hausordnung',
    generateTitle: (data) => `Hausordnung - ${data.objektBezeichnung || data.objektAdresse?.strasse || ''}`.trim() || 'Hausordnung'
  })
