...
import { ArrowLeft, FileText, Save, FileDown } from 'lucide-react'
...
export default function SondervereinbarungPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    ...
    generateTitle: (data) => `Sondervereinbarung - ${data.betreff || 'Ohne Betreff'}`
  })
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      toast({ title: 'PDF erstellt', description: 'Die Sondervereinbarung wurde als PDF heruntergeladen.' })
    } catch (error) {
