...
import { ArrowLeft, FileEdit, Plus, X, Save, FileDown } from 'lucide-react'
...
export default function NachtragsvereinbarungPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'nachtragsvereinbarung',
    generateTitle: (data) => `Nachtragsvereinbarung - ${data.mieter?.vorname || ''} ${data.mieter?.nachname || ''}`.trim() || 'Nachtragsvereinbarung'
  })
