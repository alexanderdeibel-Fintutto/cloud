...
import { ArrowLeft, FileX2, Plus, Trash2, Save, FileDown } from 'lucide-react'
...
export default function WiderspruchBetriebskostenPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'widerspruch-betriebskosten',
    generateTitle: (data) => `Widerspruch Betriebskosten - ${data.mieter?.vorname || ''} ${data.mieter?.nachname || ''}`.trim() || 'Widerspruch Betriebskosten'
  })
