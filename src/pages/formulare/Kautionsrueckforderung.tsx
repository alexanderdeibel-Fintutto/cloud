...
import { ArrowLeft, Banknote, Calendar, Save, FileDown } from 'lucide-react'
...
export default function KautionsrueckforderungPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'kautionsrueckforderung',
    generateTitle: (data) => `Kautionsrückforderung - ${data.mieter?.vorname || ''} ${data.mieter?.nachname || ''}`.trim() || 'Kautionsrückforderung'
  })
