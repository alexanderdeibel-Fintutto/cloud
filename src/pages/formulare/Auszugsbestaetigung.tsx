...
import { ArrowLeft, LogOut, Save, FileDown } from 'lucide-react'
...
export default function AuszugsbestaetigungPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'auszugsbestaetigung',
    generateTitle: (data) => `Auszugsbestätigung - ${data.mieter?.vorname || ''} ${data.mieter?.nachname || ''}`.trim() || 'Auszugsbestätigung'
  })
