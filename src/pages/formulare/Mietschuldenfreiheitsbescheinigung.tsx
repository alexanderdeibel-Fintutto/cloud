...
import { ArrowLeft, FileCheck, CheckCircle, Save, FileDown } from 'lucide-react'
...
export default function MietschuldenfreiheitsbescheinigungPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'mietschuldenfreiheitsbescheinigung',
    generateTitle: (data) => `Mietschuldenfreiheitsbescheinigung - ${data.mieter?.vorname || ''} ${data.mieter?.nachname || ''}`.trim() || 'Mietschuldenfreiheitsbescheinigung'
  })
