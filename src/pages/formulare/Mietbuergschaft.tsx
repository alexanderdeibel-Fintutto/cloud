...
import { ArrowLeft, Shield, Save, FileDown } from 'lucide-react'
...
export default function MietbuergschaftPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'mietbuergschaft',
    generateTitle: (data) => `Mietbürgschaft - ${data.buerge?.vorname || ''} ${data.buerge?.nachname || ''}`.trim() || 'Mietbürgschaft'
  })
