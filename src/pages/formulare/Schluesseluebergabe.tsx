...
import { ArrowLeft, Key, Plus, Trash2, Save, FileDown } from 'lucide-react'
...
export default function SchluesseluebergabePage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'schluesseluebergabe',
    generateTitle: (data) => `Schlüsselübergabe - ${data.empfaenger?.vorname || ''} ${data.empfaenger?.nachname || ''}`.trim() || 'Schlüsselübergabe'
  })
