...
import { ArrowLeft, Clock, Save, FileDown } from 'lucide-react'
...
export default function ZeitmietvertragPage() {
  ...
  const { handleSave, documentId } = useDocumentSave({
    type: 'zeitmietvertrag',
    generateTitle: (data) => `Zeitmietvertrag - ${data.mieter?.vorname || ''} ${data.mieter?.nachname || ''}`.trim() || 'Zeitmietvertrag'
  })
