 ...
 * - Schreibt "online" / "offline" / "redirect" / "error" in Spalte D
 * - Schreibt HTTP Status Code in Spalte E (optional)
 * - Schreibt Redirect-Ziel in Spalte F (optional)
 * - Prüft auf Google Analytics/GTM und schreibt in Spalte G
 * - Prüft Title Tag und schreibt in Spalte H
 * ============================================================
 */
...
function checkAllUrls() {
  ...
  sheet.getRange(1, CONFIG.HTTP_CODE_COLUMN).setValue("HTTP Code");
  sheet.getRange(1, CONFIG.REDIRECT_COLUMN).setValue("Redirect");
  sheet.getRange(1, CONFIG.GA_GTM_COLUMN).setValue("GA/GTM");
  sheet.getRange(1, CONFIG.TITLE_COLUMN).setValue("Title");
  ...
  for (var i = 0; i < urls.length; i++) {
    ...
    sheet.getRange(row, CONFIG.STATUS_COLUMN).setValue(result.status);
    sheet.getRange(row, CONFIG.HTTP_CODE_COLUMN).setValue(result.httpCode);
    sheet.getRange(row, CONFIG.REDIRECT_COLUMN).setValue(result.redirectUrl);
    sheet.getRange(row, CONFIG.GA_GTM_COLUMN).setValue(result.gaGtm);
    sheet.getRange(row, CONFIG.TITLE_COLUMN).setValue(result.title);

    // Farbe setzen
    var color = getStatusColor(result.status);
    sheet.getRange(row, CONFIG.STATUS_COLUMN).setBackground(color);
...
function getStatusColor(status) {
  if (status === "online") return "#b7e1cd";           // Grün
  if (status.indexOf("online") >= 0) return "#d9ead3"; // Helles Grün (redirect)
  if (status === "offline") return "#f4c7c3";           // Rot
  if (status.indexOf("redirect") >= 0) return "#fce8b2"; // Gelb
  if (status.indexOf("404") >= 0) return "#f4c7c3";    // Rot
  if (status.indexOf("403") >= 0) return "#fce8b2";    // Orange
  if (status.indexOf("error") >= 0) return "#f4c7c3";  // Rot
  return "#ffffff";
}
