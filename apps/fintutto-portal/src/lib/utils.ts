...
export function calculateMietpreisbremse(
  ...
  const maxAllowedMiete = ortsueblicheMiete * 1.1
...
export function getFormulareAppUrl(formType: string, prefillData?: Record<string, string>): string {
  // In the unified portal, formulare are internal routes
  const baseUrl = '/formulare'
  const url = `${baseUrl}/${formType}`

  if (prefillData) {
    const params = new URLSearchParams(prefillData)
