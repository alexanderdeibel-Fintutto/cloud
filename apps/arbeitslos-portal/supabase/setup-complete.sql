-- =============================================================================
-- COMPLETE SETUP für BescheidBoxer (arbeitslos-portal)
-- =============================================================================
...
CREATE INDEX IF NOT EXISTS idx_amt_users_auth_id ON amt_users(auth_id);
CREATE INDEX IF NOT EXISTS idx_amt_users_email ON amt_users(email);
CREATE INDEX IF NOT EXISTS idx_amt_users_stripe_customer ON amt_users(stripe_customer_id);
...
CREATE INDEX IF NOT EXISTS idx_amt_chat_logs_user ON amt_chat_logs(user_id);
CREATE INDEX IF NOT EXISTS idx_amt_chat_logs_created ON amt_chat_logs(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_amt_scans_user ON amt_scans(user_id);
...
CREATE INDEX IF NOT EXISTS idx_amt_letters_user ON amt_letters(user_id);

-- ============================================================
-- 5. WIDERSPRUCH TRACKER
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_amt_widersprueche_user ON amt_widersprueche(user_id);
CREATE INDEX IF NOT EXISTS idx_amt_widersprueche_fristende ON amt_widersprueche(fristende);
...
CREATE INDEX IF NOT EXISTS idx_amt_forum_posts_category ON amt_forum_posts(category);
CREATE INDEX IF NOT EXISTS idx_amt_forum_posts_created ON amt_forum_posts(created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_amt_forum_replies_post ON amt_forum_replies(post_id);
...
CREATE INDEX IF NOT EXISTS idx_amt_credit_tx_user ON amt_credit_transactions(user_id);
...
DROP POLICY IF EXISTS "Users see own profile" ON amt_users;
CREATE POLICY "Users see own profile" ON amt_users
  FOR SELECT USING (auth.uid() = auth_id);

DROP POLICY IF EXISTS "Users update own profile" ON amt_users;
CREATE POLICY "Users update own profile" ON amt_users
  FOR UPDATE USING (auth.uid() = auth_id);

DROP POLICY IF EXISTS "Users insert own profile" ON amt_users;
CREATE POLICY "Users insert own profile" ON amt_users
  FOR INSERT WITH CHECK (auth.uid() = auth_id);

DROP POLICY IF EXISTS "Users see own chats" ON amt_chat_logs;
...
DROP POLICY IF EXISTS "Users see own scans" ON amt_scans;
CREATE POLICY "Users see own scans" ON amt_scans
  FOR ALL USING (user_id IN (SELECT id FROM amt_users WHERE auth_id = auth.uid()));

DROP POLICY IF EXISTS "Users see own letters" ON amt_letters;
CREATE POLICY "Users see own letters" ON amt_letters
  FOR ALL USING (user_id IN (SELECT id FROM amt_users WHERE auth_id = auth.uid()));

DROP POLICY IF EXISTS "Users manage own widersprueche" ON amt_widersprueche;
CREATE POLICY "Users manage own widersprueche" ON amt_widersprueche
  FOR ALL USING (user_id IN (SELECT id FROM amt_users WHERE auth_id = auth.uid()));

DROP POLICY IF EXISTS "Anyone can read forum posts" ON amt_forum_posts;
CREATE POLICY "Anyone can read forum posts" ON amt_forum_posts
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Authenticated users can create posts" ON amt_forum_posts;
CREATE POLICY "Authenticated users can create posts" ON amt_forum_posts
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Authors can update own posts" ON amt_forum_posts;
...
DROP POLICY IF EXISTS "Anyone can read forum replies" ON amt_forum_replies;
CREATE POLICY "Anyone can read forum replies" ON amt_forum_replies
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Authenticated users can reply" ON amt_forum_replies;
CREATE POLICY "Authenticated users can reply" ON amt_forum_replies
  FOR INSERT WITH CHECK (auth.uid() IS NOT NULL);

DROP POLICY IF EXISTS "Users see own credit transactions" ON amt_credit_transactions;
...
DROP TRIGGER IF EXISTS trigger_reset_chat_count ON amt_users;
...
DROP TRIGGER IF EXISTS trigger_updated_at_forum_posts ON amt_forum_posts;
...
DROP TRIGGER IF EXISTS trigger_increment_reply_count ON amt_forum_replies;
...
-- ============================================================
-- 11. ADMIN-ACCOUNT anlegen
-- ============================================================
-- WICHTIG: Bevor dieser Teil läuft, musst du den Auth-User anlegen!
-- Dashboard → Authentication → Users → "Add user" → "Create new user"
--   Email: alexander@fintutto.de
--   Password: (wähle ein sicheres Passwort)
--   "Auto Confirm User" ANHAKEN
--
-- Danach legt dieses Script den Profile-Eintrag mit Vollschutz-Tier an:
...
FROM auth.users au
WHERE au.email = 'alexander@fintutto.de'
...
-- Credit-Transaktion für Audit-Log
...
-- Verifizierung - sollte eine Zeile mit plan='vollschutz' zurückgeben
SELECT email, plan, credits_current, period_end
FROM amt_users
WHERE email = 'alexander@fintutto.de';
